...
int main(int argc, char *argv[])
{
    ...
    while ((ch = getopt(argc, argv, "d:D:p:P:l:L:VvHhI:i:AaCcq")) != -1)
    {
        ...
        case 'P':
            if (countryinfo.codepagesnumber < 16)
            {
                ...
                ProgError(1, "MKMSGF: More than 16 codepages entered");
            break;

        case 'l':
        case 'L':
            if (proclang)
                ProgError(1, "MKMSGF: Syntax error L option");
    ...
    if ((argc == optind) && !procinfile)
        ProgError(1, "MKMSGF: no input file");
    ...
    do
    {
        ...
        for (; *buffptr != NULL;)
        {
            ...
            if (msgsizechk == 0)
                break; // if 0 then I sure hope I am done
        }
    ...
    if (verbose)
    {
        ...
        printf("  strOut    = %s\n", outputfile);
        printf("  StrIncDir = ");
        if (_UnknOpt1 || _UnknOpt2)
            printf("%s\n", includepath);
        else
            printf("\n");
        printf("  CodePages = ");
        for (int i = 0; i < countryinfo.codepagesnumber; i++)
            printf("%d ", countryinfo.codepages[i]);
        ...
        printf("  CP_type   = ");
        if (countryinfo.bytesperchar == 1)
            printf("SBCS\n");
        else if (countryinfo.bytesperchar == 2)
            printf("DBCS\n");
        else
            printf("unknown");
...
int DecodeLangOpt(char *dargs)
{
    ...
    {
        if (langinfo[i].langfam == countryinfo.langfamilyID)
            if (langinfo[i].langsub == countryinfo.langversionID)
                verchk = 1;
    }
...
void GetInputFile(void)
{
    ...
    { // access in file ?
        if (strlen(ext) == 0)
            strcat(inputfile, ".txt"); // no! try with txt ext
        if (access(inputfile, F_OK) != 0)
        {
